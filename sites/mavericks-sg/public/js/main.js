/**
 * Mavericks Consulting — Main Frontend Module
...
const SERVICE_ICONS = {
  strategy: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>`,
  digital:  `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>`,
  agile:    `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>`,
  coaching: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>`,
};
  ...
  try {
    ...
    grid.innerHTML = json.data.map(svc => `
      <article class="service-card" aria-label="${svc.title}">
        <div class="service-icon" aria-hidden="true">${SERVICE_ICONS[svc.icon] ?? SERVICE_ICONS.strategy}</div>
        <h3>${svc.title}</h3>
        <p>${svc.description}</p>
...
const formatDate = iso => new Date(iso).toLocaleDateString('en-SG', {
